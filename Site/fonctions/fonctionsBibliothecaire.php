        ...
        while ($test !=false) {
          ...
          if( isset($row[2])) {
            ...
            echo '<td> <form method="post" action="monCompte.php?inf=enregistrerEmprunt">
                <input type="hidden" name ="pseudo_emprunt" id="pseudo_emprunt" value="'.$_REQUEST['pseudo'].'"> </input>
                  <input type="hidden" name ="ide" id="ide" value="'.$row[0].'"> </input>
...
?><?php
  function enregistrer_emprunt() {
      if( empty($_REQUEST['pseudo']) ) {
      echo '<div class="ligne" id="enr_emprunt" style = "display: flex; margin: 10px;">

          Le pseudo du client emprunteur
          <form method="post" action="monCompte.php?inf=enregistrerEmprunt">
          <input type="text" name ="pseudo" id="pseudo"> </input>
        ...
        while ($test !=false) {
          ...
          if( $row[2] != NULL) {
            ...
            echo '<td> <form method="post" action="monCompte.php?inf=enregistrerEmprunt">
                  ...
                  <input style="width:100%" type="submit" name="emprunt" value="Valider"/>
                  </form>
            </td></tr>';
          }
          $test = ($row = oci_fetch_array($ordre, OCI_BOTH));
  ...
  function enregistrer_Retour() {
    if( empty($_REQUEST['retour']) ) {
      echo '<div class="ligne" id="enr_retour" style = "display: flex; margin: 10px;">
      <form method="post" action="monCompte.php?inf=enregistrerRetour">
              <label for="ido_retour"> Identifiant de l\'oeuvre :</label>"
              <input type="number" name ="ido_retour" id="ido_retour" disabled> </input>
                  
                  <label for="ide_retour"> Identifiant de l\'emprunt :</label>"
                  <input type="number" name ="ide_retour" id="ide_retour" disabled> </input>

                  <input style="width:100%" type="submit" name="retour" value="Enregistrer le retour"/>
                  </form></div>';
        } else {
          $connexion = oci_connect('c##lizri_a', 'lizri_a', 'dbinfo');
      $texte = "begin aRenduOeuvre(:ido, :ide) ; end;";
  ...
  function annuler_Penalite() {
    if( empty($_REQUEST['annuler']) ) {
      echo '<div class="ligne" id="anul_penal" style = "display: flex; margin: 10px;">
      <form method="post" action="monCompte.php?inf=annulerPenal">
              <label for="idp"> Identifiant de la pénalité :</label>
              <input type="number" name ="idp" id="idp" disabled> </input>
                  
                  <input style="width:100%" type="submit" name="annuler" value="Annuler la pénalité"/>
                  </form></div>';
        } else {
          $connexion = oci_connect('c##lizri_a', 'lizri_a', 'dbinfo');
      $texte = "begin supprimePenalite(:idp) ; end;";
  ...
  function remet_enRayon() {
    ...
    $texte = "begin remetEnRayon ; end;";
      ...
      oci_free_statement($ordre);
    oci_close($connexion);
    echo '<div> Toutes les oeuvres ont été remis en rayon </div>';
  }
  ...
  function mAJPenalites() {
    ...
    $texte = "begin creationEtMAJPenalites ; end;";
      ...
      oci_free_statement($ordre);
    oci_close($connexion);
    echo '<div> Création et mise à jour des pénalités effectué avec succès </div>';
  }
