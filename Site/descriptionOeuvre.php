        ...
        <?php
        	...
        	if (!empty($_REQUEST['ido']) and !empty($_REQUEST['add']) and $_REQUEST['add']==true ){
				if (!empty($_SESSION['pseudo'])){	//Si connecté
					...
					if ($msg=='Ok')
						echo "Ajout Réussi !</div>";
					else
						echo "Echec Ajout ! ".$msg."</div>";
				}else{
...
//l'oeuvre
			...
			if ($test==false)
	            ...
	            echo '<div class="section" style="padding-bottom:20px;">
                      <button class="btnOeuvre"> <a href="descriptionOeuvre.php?reference=203012&add=true&ido='.$row[6].'"> Add to cart <i class="fas fa-shopping-cart"></i></a></button>
                    </div></div>';
